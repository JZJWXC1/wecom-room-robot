...
SHEET_TERMS = ("房源表", "表发", "发一下表")
VIDEO_TERMS = ("视频", "原视频", "原片", "视", "笔记")
IMAGE_TERMS = ("图片", "照片", "图")
VIEWING_TERMS = (
    "密码",
    "看房方式",
...
def post_message(user_id: str, content: str) -> dict:
    payload = json.dumps(
...
def actions_from(data: dict) -> list[str]:
    result = []
...
def count_value(tool: dict, *keys: str) -> int:
    for key in keys:
        value = tool.get(key)
        if value is None:
            continue
...
def has_missing_media(tool: dict, reply: str, kind: str) -> bool:
...
def validation_status(data: dict) -> str:
    candidates = []
...
def evaluate(question: str, data: dict) -> list[str]:
    ...
    if contains_any(question, SHEET_TERMS):
        if count_value(tool, "inventory_image_count") <= 0 and "send_inventory_sheet" not in action_text and not contains_any(reply, ("房源表", "表")):
...
def compact_record(window_index: int, window_name: str, turn_index: int, question: str, data: dict, elapsed: float, error: str = "") -> dict:
    tool = data.get("tool_evidence") if isinstance(data.get("tool_evidence"), dict) else {}
...
def write_summary(records: list[dict], started: float, final: bool, environment: dict) -> None:
...
async def run_window(index: int, scenario: dict, records: list[dict], lock: asyncio.Lock, started: float, environment: dict) -> None:
    ...
    for turn_index, question in enumerate(scenario["turns"], start=1):
        ...
        async with lock:
            ...
            print(json.dumps({"event": "turn", "done": len(records), "window": index, "turn": turn_index, "ok": record["ok"], "failures": record["failures"]}, ensure_ascii=False), flush=True)
...
async def main() -> int:
    ...
    print(json.dumps({"event": "start", "run_id": RUN_ID, "turns": sum(len(item["turns"]) for item in SCENARIOS), "concurrency": MAX_CONCURRENT_WINDOWS, "environment": environment}, ensure_ascii=False), flush=True)

    async def guarded(index: int, scenario: dict) -> None:
    ...
    print(json.dumps({"event": "done", "failed": failed, "summary": str(SUMMARY_PATH), "result": str(RESULT_PATH)}, ensure_ascii=False), flush=True)
