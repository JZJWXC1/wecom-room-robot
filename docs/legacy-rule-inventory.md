...
## M1D2B2 Local Primary Rehearsal Legacy Removal Gate

M1D2B2 只做本地 primary 全链路演练、故障回退、cutover readiness evaluator 和 runbook，不进入生产切换，因此本轮不删除旧生产路径。

本轮新增收束：

- `inventory_snapshot_cutover.py` 提供虚构本地 Snapshot 构建、primary replay、Legacy/Snapshot parity、rollback rehearsal 和 performance summary。
- offline、shadow、primary replay 复用同一个 `scan_public_artifacts_for_sensitive_text` 安全扫描语义。
- `sanitize_for_log` 保留 `ird_/evd_` 内部安全 ID，同时普通文本中的手机号、password context、token context 和 canary 会脱敏。
- pointer、manifest、private viewing secret、PNG 故障注入均在本地测试中证明不会产生半轮混用或不安全发送。

本轮明确保留：

| Legacy/兼容项 | M1D2B2 状态 | removal_milestone | 保留原因 |
| --- | --- | --- | --- |
| `InventoryService` | 保留 | approved primary cutover 通过并观察稳定后 | 生产客户读取仍未获批切 primary |
| 旧 `data/inventory_cache.csv` | 保留 | primary 稳定性复审后 | legacy_whole_request fallback 和生产回滚 |
| 旧 `data/rewrite_inventory_index.json` | 保留 | primary 稳定性复审后 | rewrite/prompt 等价回归仍需 legacy 基线 |
| 旧 `room_database/inventory_*.png` | 保留 | Snapshot PNG 接管发送且回滚窗口结束后 | 客户房源表发送仍未切 Snapshot |
| `LegacyInventoryReadProvider` | 保留 | primary 成为唯一批准读取源后 | golden parity 和整体 fallback 契约 |

删除前仍需：

- 获得明确 `APPROVE_DEPLOY`。
- 生产 primary cutover runbook 执行并可回滚。
