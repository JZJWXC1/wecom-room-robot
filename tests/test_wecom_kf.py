...
def test_llm2_production_second_retry_does_not_replace_media_actions_with_inventory_list(monkeypatch) -> None:
    async def run_case() -> None:
        ...
        assert result["needs_planner_retry"] is False
        assert tool_evidence["deterministic_reply_source"] == "llm2_production_safe_fallback"
        assert tool_evidence.get("llm2_production_grounded_fallback_used") is not True
        assert tool_evidence["suppress_actions"] is True
        assert tool_evidence["outbound_package"]["video_paths"] == []
        assert "有的，我查到这些还在租" not in result["reply"]
        assert "不能乱发视频或图片" in result["reply"]
