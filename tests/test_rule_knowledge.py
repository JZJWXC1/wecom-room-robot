...
def test_builtin_rule_cards_cover_planner_tool_grounded_reply() -> None:
    service = RuleKnowledgeService()
...
def test_builtin_rule_cards_cover_planner_context_binding() -> None:
    service = RuleKnowledgeService()
...
def test_builtin_rule_cards_cover_budget_payment_scope() -> None:
    service = RuleKnowledgeService()
...
def test_selfcheck_prompt_includes_outbound_rule_cards(monkeypatch, tmp_path) -> None:
    ...
    _write_card(
        ...
        intents="media",
        triggers="视频",
        content="自检不通过时必须拦住所有动作回到 Planner。",
    )
    captured: dict[str, object] = {}
    ...
    assert "自检不通过时必须拦住所有动作回到 Planner" in prompt
