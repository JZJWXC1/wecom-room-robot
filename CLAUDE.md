...
9. **重大设计先比选**：选库、定表结构、定架构、调整跨模块契约或 RAG 阶段职责前，
   必须先给出 2-3 个方案对比，说明取舍、风险、测试/迁移成本，再进入实现。
10. **模块决策留痕**：每完成一个模块，用 3-5 句话总结设计思路、边界和验证方式，
    追加到 `docs/DECISIONS.md`；文档或测试模块也要记录影响范围。
11. **平台边界说清楚**：涉及微信、企业微信、飞书、阿里云等平台限制时，
    明确标注这是平台限制还是本项目设计选择，避免把平台约束误写成业务可选项。
